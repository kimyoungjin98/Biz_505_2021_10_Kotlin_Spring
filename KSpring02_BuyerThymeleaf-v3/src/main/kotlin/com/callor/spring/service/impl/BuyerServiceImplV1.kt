...
class BuyerServiceImplV1(val bRepo:BuyerRepository ):BuyerService {
    ...
    override fun selectAll(): Array<Buyer> {
        return ConfigString.BUYER_LIST
    }

    override fun findById(userid: String): Buyer {
        val findUser = ConfigString.BUYER_LIST.filter { buyer-> buyer.userid == userid }
        return findUser[0]
    }

    override fun findByName(name: String): Array<Buyer> {
        val userNum = ConfigString.RND.nextInt(ConfigString.BUYER_LIST.size)
        return arrayOf(ConfigString.BUYER_LIST[userNum])
    }
